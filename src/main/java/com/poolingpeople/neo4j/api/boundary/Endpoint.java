...
public class Endpoint {
    ...
    public URI getCypherEndpoint_TxBegin() {
        try {
    ...
    public URI getCypherEndpoint() {
        try {
            return new URI("http://" + host + ":" + port + "/db/data/transaction/" + statesManager.getCommitValue());
        } catch (URISyntaxException e) {
            throw new RuntimeException(e);
        }
    }

    public URI getCypherEndpoint_TxCommit() {
        try {
            return new URI("http://" + host + ":" + port + "/db/data/transaction/" + statesManager.getCommitValue() + "/commit");
        } catch (URISyntaxException e) {
            throw new RuntimeException(e);
        }
    }
